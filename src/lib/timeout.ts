...
export class Timeout {
    ...
    static set(...args: any[]): boolean {
        ...
        const invoke = () => {
            Timeout.metadata[key].executed = new Date().getTime();
            callback(...params);
        };
    ...
    static create(...args: any[]): boolean {
        if (args.length === 0) {
    ...
    static executed(key: string): boolean {
        return Timeout.exists(key) && !!Timeout.metadata[key].executed;
    }
    ...
    static lastExecuted(key: string): Date {
        if (!Timeout.executed(key)) {
            return null
        } else {
            return new Date(Timeout.metadata[key].executed)
        }
    }
